...
(function () {
    ...
    var AI_HIST_KEY = 'apex_clothing_ai_history';
    ...
    function projectCard(meta, isMine) {
        ...
        if (meta.thumb) {
            ...
            thumbBox.appendChild(icon('fa-shirt'));
        }
    ...
    function buildPrompt(userPrompt, opts) {
        ...
        if (opts.target === 'selection') {
            ...
            p = 'A flat clothing texture patch that completely fills the frame, edge to edge: '
            ...
            /* Strong constraints — the model tends to (a) fill the
               transparent background, (b) invent/duplicate logos, and
               (c) "correct" rotated/vertical text back to horizontal.
               The alpha mask handles (a) as a safety net; this wording
               tackles all three up front. */
            p = 'This is a flat GAME CLOTHING UV TEXTURE SHEET on a transparent background. '
                + 'It is an unwrapped UV layout, so pieces, text and logos legitimately appear '
                + 'ROTATED, VERTICAL, SIDEWAYS, MIRRORED or UPSIDE-DOWN — this is correct and intentional. '
                + 'Read the text in every orientation and keep each word, number and logo in its EXACT '
                + 'original orientation, angle, position and reading direction. Do NOT rotate, straighten, '
                + 're-typeset, flip or re-flow any text or logo. '
                + 'Edit ONLY the fabric of the garment pieces, keeping the EXACT same layout, '
    ...
    function bindAiPane() {
        ...
        if (chipsToggle && chipsBox) {
            var collapsed = false;
            try { collapsed = localStorage.getItem('apex_clothing_ai_chips_hidden') === '1'; } catch (e) { /* ok */ }
            function applyChips() {
            ...
            chipsToggle.addEventListener('click', function () {
                collapsed = !collapsed;
                try { localStorage.setItem('apex_clothing_ai_chips_hidden', collapsed ? '1' : '0'); } catch (e) { /* ok */ }
                applyChips();
            });
        }
    ...
    var QF_MATERIALS = [
        'on brushed satin', 'on matte ripstop weave', 'on soft velvet',
        'on woven denim', 'on glossy latex', 'on knit wool', 'on silk twill',
        'on distressed leather', 'on heavy canvas', 'on iridescent nylon'
    ];
    ...
    function generateTheme() {
        ...
        return {
            ...
            prompt: adj + ' ' + style + ' clothing texture, flat fabric print of '
                + motif + ' in ' + palette + ' ' + mat
                + ', elegant repeat, high detail, no human, no mannequin.'
        };
    }
    ...
    var TPL_BASE = 'https://cfx-nui-apex_clothing/';
    ...
    function normalizeLocal(raw, i) {
        ...
        return {
            ...
            componentKey: raw.componentKey || 'jbib',
            glb: raw.glb || '',
    ...
    function fetchLocalCatalog() {
        /* cache-bust: FiveM's CEF caches templates.json, so after you
           ...
           reload every time. */
        var bust = 'https://cfx-nui-apex_clothing/templates.json?t=' + Date.now();
    ...
    function renderTemplateGrid() {
        ...
        list.forEach(function (tpl) {
            ...
            var ph = icon('fa-shirt'); ph.classList.add('tpl-thumb-ph');
    ...
    function renderTextureVariants() {
        ...
        if (cur) { cur.textContent = String(active.id || '?').toUpperCase(); }
    ...
    var PROPS_COLLAPSE_KEY = 'apex_clothing_props_collapsed';
    ...
    var SNAP_KEY = 'apex_clothing_snaps';
    ...
    var SUPPORT_CHAT_KEY = 'apex_clothing_support_chat';
    ...
    var EXPORT_NOTES = {
        package: ['export_note_package',
            'Creates exports/<name>/ — a complete resource. Copy it into your server and add "ensure <name>".'],
        ytd: ['export_note_ytd',
            'Stages the PNG in tools/ytd_in/. Then run tools/make_ytd.bat on this PC to get the real .ytd + .ydd pair.'],
    ...
    function bindExportPane() {
        ...
        function renderExportResult(p) {
            ...
            if (p.mode === 'live') {
                ...
                box.appendChild(el('div', 'result-note',
                    t('export_wear_live', "Put the same garment on with any clothing menu — it is wearing your design now, for everyone on the server.")));
                box.appendChild(resultLabel(t('export_undo_label', 'TO UNDO IT')));
                box.appendChild(copyRow('/clothingliveclear'));
                return;
            }
            ...
            box.appendChild(resultLabel(t('export_how_label', 'HOW TO SEE IT')));
            box.appendChild(el('div', 'result-note',
                t('export_wear_package', "This pack REPLACES the texture of a garment that already exists in the game, so there is no new shop item to add. Put that same garment on and it wears your design.")));
        }
    ...
    var THEME_KEY = 'apex_clothing_theme';
    ...
    function openSettings() {
        ...
        arCk.checked = localStorage.getItem('apex_clothing_autorotate') === '1';
        ...
        arCk.addEventListener('change', function () {
            localStorage.setItem('apex_clothing_autorotate', arCk.checked ? '1' : '0');
    ...
    function startTour() {
        ensureTourEls();
        try { localStorage.setItem('apex_clothing_tour_seen', '1'); } catch (e) { /* ok */ }
        tourGo(0);
    }
    ...
    function bindTutorial() {
        ...
        App.on('open', function () {
            try {
                if (localStorage.getItem('apex_clothing_tour_seen') !== '1') {
